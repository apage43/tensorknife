...
impl<F> PyValueVisitor for F
where
    F: FnMut(&PyValue) -> bool,
{
    fn visit(&mut self, item: &PyValue) -> bool {
        self(item)
    }
...
pub fn unpickle(input: &mut impl std::io::BufRead) -> Result<PyValue> {
    ...
    loop {
        ...
        match key {
            ...
            b'c' => {
                ...
                stack.push(PyValue::Global(
                    module.trim_end().to_string(),
